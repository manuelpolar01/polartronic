...
export const DEFAULT_PROJECTS = [
  { id: 'p1', client: 'Maison Noir',  industry: 'Restaurante', title: 'Rediseño Digital Completo',      description: 'Sitio web inmersivo con sistema de reservas integrado y menú interactivo.', results: '+300% reservas online · +45% ticket promedio',       image: 'https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?auto=format&fit=crop&w=800&q=80', url: '#', order: 0 },
  { id: 'p2', client: 'VitaCare',     industry: 'Salud',       title: 'Plataforma de Citas Médicas',    description: 'Portal completo con agenda online, historial de pacientes y telemedicina.',  results: '+220% pacientes nuevos · 0 llamadas de agendamiento', image: 'https://images.unsplash.com/photo-1579684385127-1ef15d508118?auto=format&fit=crop&w=800&q=80', url: '#', order: 1 },
  { id: 'p3', client: 'Urban Cuts',   industry: 'Barbería',    title: 'Sistema de Reservas + Branding', description: 'Identidad visual urbana y agenda digital con recordatorios automáticos.',     results: 'Agenda llena en 2 semanas · +150% clientes recurrentes', image: 'https://images.unsplash.com/photo-1596728325488-58c87691e9af?auto=format&fit=crop&w=800&q=80', url: '#', order: 2 },
  { id: 'p4', client: 'Luxe Fashion', industry: 'Moda',        title: 'E-commerce de Lujo',             description: 'Showroom digital con experiencia premium y checkout optimizado.',           results: '+180% conversión · Tasa de rebote -60%',               image: 'https://images.unsplash.com/photo-1445205170230-053b83016050?auto=format&fit=crop&w=800&q=80', url: '#', order: 3 },
]
...
// ─── Fusiona site config con defaults ────────────────────────────────
...
export function useSiteData() {
  ...
  // Inicializa SIEMPRE desde caché si existe — render síncrono sin flash
  const [site,         setSite]         = useState(() => mergeSite(cached?.site))
  ...
  useEffect(() => {
    ...
    async function hydrate() {
      ...
      try {
        ...
        const freshEco  = eco.length  ? sort(eco)  : DEFAULT_ECOSYSTEMS
        const freshProj = proj.length ? sort(proj) : DEFAULT_PROJECTS
        const freshTest = test.length ? sort(test) : DEFAULT_TESTIMONIALS
        const freshSvc  = svc.length  ? sort(svc)  : DEFAULT_SERVICES
        ...
        console.warn('[useSiteData] Firebase error — keeping cache/defaults:', e.message)
      } finally {
