...
const EMPTY = {
  ...
  ctaLabel: '', ctaLink: '',
  detailHtml: '',      // HTML personalizado opcional
  extraText: '',
  order: 0,
}

// Convierte array JS → string JSON legible para el textarea
const arrToStr = (val) => {
...
// Convierte string de textarea (una línea por ítem) → JSON array string para guardar
...
export default function EcosystemsTab({ data }) {
  ...
  const [featText,  setFeatText] = useState('')   // textarea crudo para features
  const [saving,    setSaving]   = useState(false)
  const [deleting,  setDeleting] = useState(null)
  const [tab,       setTab]      = useState('basic') // 'basic' | 'content' | 'cta'
  ...
  /* ── Estilos de tabs internas del form ── */
  const tabStyle = (id) => ({
            ...
            <>
              ...
              <FieldGroup label="Texto adicional (opcional, aparece en el modal)">
                <textarea className="admin-textarea" value={form.extraText}
                  ...
                  rows={3} placeholder="Texto descriptivo adicional que verá el usuario al abrir el detalle…" />
              </FieldGroup>
            </>
          )}
          ...
          {tab === 'cta' && (
            <>
              ...
              <FieldGroup
                ...
                hint="Si pegas HTML aquí, reemplaza la vista automática del modal. Útil para incrustar tablas comparativas, iframes, etc. Deja vacío para usar la vista automática."
              >
                <textarea
                  className="admin-textarea"
