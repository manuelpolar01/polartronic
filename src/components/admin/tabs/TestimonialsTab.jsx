...
export default function TestimonialsTab({ data }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await saveTestimonial(id, form)
      if (editing === 'new') {
        setTestimonials(prev => [...prev, { ...form, id: result.id }])
      } else {
        ...
        {testimonials.map((t, idx) => (
          <div key={t.id || idx} style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <button onClick={() => startEdit(t, idx)} style={{
                background: 'rgba(255,255,255,0.05)',
                border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.6)',
                padding: '6px 12px', borderRadius: 6, cursor: 'pointer', fontSize: 11,
              }}>✏️</button>
              <button onClick={() => handleDelete(t, idx)} disabled={deleting === idx} style={{
