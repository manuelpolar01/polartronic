...
export default function ServicesTab({ data }) {
  ...
  const handleDelete = async (svc, idx) => {
    setDeleting(idx)
    try {
      if (svc.id && !svc.id.startsWith('s')) await deleteService(svc.id)
    ...
    <div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {services.map((svc, idx) => (
          <div key={svc.id || idx} style={{ background:'rgba(255,255,255,0.03)',
            ...
            <div style={{ display:'flex', gap:8 }}>
              <button onClick={() => startEdit(svc, idx)}
                style={{ background:'rgba(255,255,255,0.05)', border:'1px solid rgba(255,255,255,0.1)',
                  color:'rgba(255,255,255,0.6)', padding:'6px 14px', borderRadius:6,
                  cursor:'pointer', fontSize:12 }}>✏️ Editar</button>
              <button onClick={() => handleDelete(svc, idx)} disabled={deleting === idx}
                style={{ background:'rgba(255,60,60,0.08)', border:'1px solid rgba(255,60,60,0.2)',
                  color:'#ff3c3c', padding:'6px 14px', borderRadius:6, cursor:'pointer', fontSize:12 }}>
                {deleting === idx ? '...' : '🗑 Borrar'}
              </button>
            </div>
          </div>
